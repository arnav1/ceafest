<?php
	...
	$q = "SELECT COUNT(*) as total FROM users_quizzes WHERE (attempted=1 and disqualified=0) ORDER BY marks, time_remained LIMIT 0,400;";
	$r = mysql_query($q);
	$l = mysql_fetch_assoc($r);
	$total = $l['total'];
	
	$q = "SELECT * FROM users_quizzes WHERE (attempted=1 and disqualified=0) ORDER BY marks, time_remained LIMIT 0,400;";
	$r = mysql_query($q);
